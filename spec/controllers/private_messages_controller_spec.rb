    ...
    context "with valid input" do
      ...
      context "when a volunteer submits his/her work for review" do
        ...
        it "renders the current user's inbox" do
          ...
          conversation1.private_messages << [message1, message2]
          post :create, contract_id: contract1.id, private_message: {recipient_id: alice.id, sender_id: bob.id, subject: "Contract Complete", body: "This work is done"}
        ...
        it "starts another conversation with the administrator" do
          set_current_user(bob)
          alice = Fabricate(:organization_administrator, organization_id: nil, first_name: "Alice", last_name: "Smith", user_group: "nonprofit")
          huggey_bear = Fabricate(:organization, user_id: alice.id) 
          alice.update_columns(organization_id: huggey_bear.id)
          application1 = Fabricate(:volunteer_application, applicant_id: bob.id, administrator_id: alice.id, project_id: word_press.id, accepted: true, rejected: false) 
          conversation1 = Fabricate(:conversation, volunteer_application_id: application1.id) 
          message1 = Fabricate(:private_message, recipient_id: alice.id, sender_id: bob.id, conversation_id: conversation1.id, subject: "Please let me join your project", body: "I'd like to contribute to your project")
          alice.conversations << conversation1
          bob.conversations << conversation1
          conversation1.private_messages << message1
          message2 = Fabricate(:private_message, recipient_id: bob.id, sender_id: alice.id, conversation_id: conversation1.id, subject: "Please let me join your project", body: "I've accepted you to join")
          conversation1.private_messages << message2
          contract1 =  Fabricate(:contract, contractor_id: alice.id, volunteer_id: bob.id, active: true, project_id: word_press.id)
          post :create, contract_id: contract1.id, private_message: {recipient_id: alice.id, sender_id: bob.id, subject: "Contract Complete", body: "This work is done"}
          
          expect(alice.reload.conversations.count).to eq(2)
        end

        it "starts a conversation with the administrator associated with the contract" do
          set_current_user(bob)
          alice = Fabricate(:organization_administrator, organization_id: nil, first_name: "Alice", last_name: "Smith", user_group: "nonprofit")
          huggey_bear = Fabricate(:organization, user_id: alice.id) 
          alice.update_columns(organization_id: huggey_bear.id)
          ...
          expect(conversation2.contract_id).to eq(contract1.id)
        end

        it "sends a private message about having submitted work to the project administrator" do
          set_current_user(bob)
          huggey_bear.update_columns(user_id: alice.id)
          application1 = Fabricate(:volunteer_application, applicant_id: bob.id, administrator_id: alice.id, project_id: word_press.id, accepted: true, rejected: false) 
          conversation1 = Fabricate(:conversation, volunteer_application_id: application1.id) 
          ...
          expect(alice.reload.private_messages.last).to eq(PrivateMessage.last)
        end
      end
    end
  end
